...
class args():
    ...
    total_epoch = 250
    model_path = 'ckpt816/cifar100-10-npcl1-unknownclasss0.1-prevtask-dev'   ### change
...
def inference_check_task(dtask, mtask, inputs, task_model):
    ...
    with torch.no_grad():
        for m in range(mtask + 1):
            ...
            #entropy[torch.roll(strong_unknown, -1)] = float('-inf')
            #unknown = (predicted == args.class_per_task)
            #entropy[unknown] *= 2
            #entropy_unknown[~unknown] = float('inf')

            if len(joint_entropy):
                # joint_entropy[-1] += 2*entropy_unknown
                # joint_entropy[-1] -= entropy
                #joint_entropy[-1][unknown] /= 2
                strong_unknown = (entropy_unknown - entropy) < 0
                #joint_entropy[-1][strong_unknown] += 0.25*(entropy_unknown - entropy)[strong_unknown]
                #joint_entropy[-1][~strong_unknown] += 1*(entropy_unknown - entropy)[~strong_unknown]
                #joint_entropy[-1][strong_unknown] /= 2

            # sout = F.softmax(outputs, 1)
            # entropy = -torch.sum(sout * torch.log(sout + 0.0001), 1)
            joint_entropy.append(entropy)
            joint_entropy_unknown.append(entropy_unknown)
            #+++++++++++++++++++++#
    ...
    #+++ unknown class +++#
    all_entropy_unknown = torch.zeros([joint_entropy_unknown[0].shape[0], mtask + 1]).cuda()
    for i in range(mtask + 1):
        all_entropy_unknown[:, i] = joint_entropy_unknown[i]


    strong_unknown = (all_entropy_unknown - all_entropy) < 0
    all_entropy[torch.roll(strong_unknown, -1, 1)] += 0.25*(all_entropy_unknown - all_entropy)[strong_unknown]
    #joint_entropy[-1][~strong_unknown] += 1*(entropy_unknown - entropy)[~strong_unknown]
    all_entropy[:,-1] += 0.1*(all_entropy_unknown - all_entropy)[strong_unknown].mean()

    # min_entropy = torch.min(all_entropy, dim=1)
    # min_entropy_unknown = torch.min(all_entropy_unknown, dim=1)
    # strong_unknown = min_entropy.values > 1 * min_entropy_unknown.values
    # min_entropy.indices[strong_unknown] = min_entropy_unknown.indices[strong_unknown] - 1
    # ctask = min_entropy.indices == dtask
    ...
    # ### dev
    # # arg_m = torch.argmin(all_entropy, axis=1)
    # # for t in range(mtask+1):
    # #     print('task {} : {}/{}'.format(t, sum(arg_m == t), len(arg_m)))

    # if mtask == 3 and dtask == 0:
    #     right = all_entropy[ctask]
    #     for m in range(mtask + 1):
    #         task_model[m].eval()
    #         outputs, _ = task_model[m](inputs)
    #         _, predicted = outputs.max(1)
    #         right = torch.cat((right, (predicted[ctask] == args.class_per_task).view(-1,1)), dim=1)
    #     print(right.shape)
    #     print(right)
    #     print()

    #     wrong = all_entropy[~ctask]
    #     for m in range(mtask + 1):
    #         task_model[m].eval()
    #         outputs, _ = task_model[m](inputs)
    #         _, predicted = outputs.max(1)
    #         wrong = torch.cat((wrong, (predicted[~ctask] == args.class_per_task).view(-1,1)), dim=1)
    #     print(wrong.shape)
    #     print(wrong)
    #     print()
