...
class args():
    ...
    total_epoch = 250
    model_path = 'ckpt816/cifar100-5-angular-check-task-dev'
...
#+++ additive angular margin loss +++#
def get_loss(model, model_prev, feat_current, feat_prev, targets):
    # Normalized Feature
    nfeat_current = F.normalize(feat_current)
    nfeat_prev = F.normalize(feat_prev)

    # Normalized Weights
    fc_current = model.linear
    fc_prev = model_prev.linear
    nweight_current = torch.tensor(fc_current.weight / torch.linalg.norm(fc_current.weight, dim=1, keepdim=True))
    nweight_prev = torch.tensor(fc_prev.weight / torch.linalg.norm(fc_prev.weight, dim=1, keepdim=True))

    # Logit
    out_current = nfeat_current @ nweight_current.transpose(0,1)
    out_prev = nfeat_prev @ nweight_prev.transpose(0,1)

    # cosine
    cosine_list = []
    for i in range(nfeat_current.shape[0]):
        cosine_list.append(torch.vdot(out_current[i], out_prev[i]))
    cosine = torch.stack(cosine_list)

    # Additive Angular Margin Penalty
    theta = torch.acos(cosine)
    out = torch.cos(theta + args.margin)
...
def get_angular_loss(feat_current, feat_prev):
    # Normalized Feature
    nfeat_current = F.normalize(feat_current)
    nfeat_prev = F.normalize(feat_prev)
    ...
    cosine_list = []
    for i in range(nfeat_current.shape[0]):
        cosine_list.append(torch.vdot(nfeat_current[i], nfeat_prev[i]))
    cosine = torch.stack(cosine_list)

    # Additive Angular Margin Penalty
    theta = torch.acos(cosine-(1e-6))     # if cosine is 1, acos often return nan
    out = torch.cos(theta + args.margin)
    #out = cosine
    #out = torch.cos(theta - args.margin)
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if task >= 1:
            for m in range(task):
                ...
                angular_loss += args.l * get_angular_loss(feat_current, feat_prev) / task
