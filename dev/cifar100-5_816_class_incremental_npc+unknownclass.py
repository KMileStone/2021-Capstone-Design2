...
class args():
    ...
    total_epoch = 250
    model_path = 'ckpt816/cifar100-10-npcl1-unknownclass-dev-temp'
...
def train(train_loader, epoch, task, model, task_model):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        #+++ unknown class +++#
        if epoch == 0:
            task_inputs[task].append(inputs[:8])
        #+++++++++++++++++++++#
    ...
    #+++ unknown class +++#
    if epoch == 0:
        task_inputs[task] = torch.cat(task_inputs[task], dim=0)

    for m in range(task):
        inputs = task_inputs[m].cuda()
        targets = torch.ones(inputs.shape[0], dtype=torch.int64).cuda() * args.class_per_task
        optimizer.zero_grad()
        outputs, feat_current = model(inputs)
    ...
    print("[Train: ], [%d/%d: ], [Accuracy: %f], [Loss: %f] [ce_loss: %f] [angular_loss: %f] [Lr: %f]"
          % (epoch, args.total_epoch, acc, train_loss / (batch_idx+task), train_ce_loss / (batch_idx+task), train_angular_loss / (batch_idx+task),
...
#+++ unknown class +++#
task_inputs = [[] for _ in range(args.num_task)]
#+++++++++++++++++++++#
